...
# Default keyword queries for big DIRECT EMPLOYERS on Workday. These boards list
# thousands of mostly-non-tech jobs; scraping newest-first buries the few tech roles
# past our page cap. So we hit Workday's own searchText with each of these terms and
# merge — narrow queries return ~1 page each, so this stays cheap. The title filter
# (filters.py) then keeps only real SWE/data/ML roles and drops non-IT engineers.
EMPLOYER_SEARCH_TERMS = [
    "software engineer", "developer", "data engineer", "data analyst",
...
def scrape_api(firm: dict, http) -> list[dict]:
    ...
    For big direct employers (firm['employer']) on a Workday-style POST board with a
    `searchText` field, we run one query per keyword and merge — see EMPLOYER_SEARCH_TERMS.
    Override per firm with `api.search_terms: [...]`, or disable with `api.search_terms: []`.
    """
    api = firm["api"]
    method = api.get("method", "GET").upper()
    is_workday_search = (
        method == "POST" and isinstance(api.get("json_body"), dict)
        and "searchText" in api["json_body"]
    )
    terms = api.get("search_terms")
    if terms is None and firm.get("employer") and is_workday_search:
        terms = EMPLOYER_SEARCH_TERMS

    if not terms:
        return _collect_api(firm, http, api, None)

    where = "json_body" if method == "POST" else "params"
    sp = api.get("search_param", "searchText" if where == "json_body" else "q")
    merged: dict = {}
    for term in terms:
        ov = {"where": where, "param": sp, "value": term}
        for j in _collect_api(firm, http, api, ov):
            merged[j.get("url") or j.get("title")] = j
    return list(merged.values())


def _collect_api(firm: dict, http, api: dict, base_override: dict | None) -> list[dict]:
    """Paginate one query (base_override = a fixed search-term override, or None)."""
    ...
    pg = api.get("paginate")
    if not pg:
        return _fetch_page(firm, http, api, [base_override] if base_override else None)[0]
    ...
    if pg.get("mode") == "cursor":
        ...
        for _ in range(pg.get("max_pages", 10)):
            ovs = [base_override] if base_override else []
            if token:
                ovs.append({"where": pg.get("in", "json_body"), "param": pg["param"], "value": token})
            page_jobs, data = _fetch_page(firm, http, api, ovs or None)
...
def _join_url(base: str, tail: str) -> str:
    """Join a url_prefix/base with an id or path. When the base ends in '=' or '?'
    (a query-param prefix like UKG '...opportunityId=' or Taleo '...job='), append
    directly — inserting a '/' there would break the URL ('=/id'). Otherwise join
    with a single '/'."""
    if base.endswith(("=", "?", "/")):
...
def _fetch_page(firm: dict, http, api: dict, override=None):
    """Returns (jobs, raw_response_json) so callers can read pagination cursors.

    `override` may be a single {where,param,value} dict or a LIST of them (so a
    search-term override and a pagination-offset override can both apply).
    """
    import copy
    method = api.get("method", "GET").upper()
    body = copy.deepcopy(api.get("json_body") or {})   # deepcopy: nested override safe
    params = dict(api.get("params") or {})
    overrides = override if isinstance(override, list) else ([override] if override else [])
    for ov in overrides:
        if ov["where"] == "json_body":
            _set_nested(body, ov["param"], ov["value"])
        else:
    ...
    for item in node:
        ...
        if not title:
            continue
        url = str(_dig(item, fields["url"]) or firm.get("search_url", ""))
        if url and not url.startswith("http"):
            # Workday's externalPath is "/job/..." WITHOUT the site segment, so a
            # host-only prefix yields a 404. Rebuild as host/<site>/job/... using the
            # site from the CXS endpoint (/wday/cxs/<tenant>/<site>/jobs).
            base = _workday_base(api["url"]) or api.get("url_prefix")
            if base:
...
def _parse_cards(html: str, firm: dict) -> list[dict]:
    ...
    for card in soup.select(sel["card"]):
        ...
        if split and location:
            location = _clean(location.split(split)[0])
        # SuccessFactors RMK tiles carry no location field — the state is in the job
        # URL slug (".../job/<City>-<Title>-OH-43604/<id>/"). Pull it out so Ohio
        # filtering works. We extract the 2-letter state (city words aren't reliably
        # separable from the title in the slug).
        if not location and firm.get("location_from_url"):
            m = re.search(r"-([A-Z]{2})-\d{4,5}", url)
            if m:
                location = m.group(1)
...
def scrape_oracle(firm: dict, http) -> list[dict]:
    """Oracle Cloud Recruiting (Fusion HCM) candidate-experience job API.

    Used by big employers on Oracle HCM — Kroger, Sherwin-Williams, FirstEnergy,
    First Solar, Diebold Nixdorf, etc. GET the recruitingCEJobRequisitions REST
    resource (needs the REST-Framework-Version header). Jobs live at
    items[0].requisitionList — which is sometimes a dict {items:[...]} and
    sometimes the list directly, so we handle both. Pagination walks an `offset=`
    token inside the `finder=findReqs;...` query segment. The job URL is built
    from the requisition Id via url_prefix (the CE site's .../job/ path).

    Config (firm['api']): url, headers, url_prefix, fields{title,location,url,posted},
    optional page_size (default 50) and pages (default 4).
    """
    api = firm["api"]
    base_url = api["url"]
    headers = api.get("headers") or {}
    prefix = api.get("url_prefix", "")
    fields = api["fields"]
    size = api.get("page_size", 50)
    pages = api.get("pages", firm.get("pages", 4))

    jobs: list[dict] = []
    for page in range(pages):
        offset = page * size
        if "offset=" in base_url:
            url = re.sub(r"offset=\d+", f"offset={offset}", base_url)
        else:                                    # inject right after the limit token
            url = re.sub(r"(limit=\d+)", rf"\1,offset={offset}", base_url, count=1)
        try:
        ...
        if not items:
            break
        rl = items[0].get("requisitionList")
        reqs = rl.get("items", []) if isinstance(rl, dict) else (rl or [])
        if not reqs:
            break
        for d in reqs:
            title = _clean(d.get(fields["title"], ""))
            if not title:
                continue
            loc = d.get(fields.get("location", ""), "")
            if isinstance(loc, list):
                loc = ", ".join(str(x) for x in loc[:1])
            jid = d.get(fields.get("url", ""), "")
            url_j = (prefix.rstrip("/") + "/" + str(jid)) if prefix else str(jid)
            jobs.append({
                "firm": firm["name"], "title": title,
                "location": _clean(str(loc)),
                "url": url_j,
                "posted": _clean(str(d.get(fields.get("posted", ""), ""))),
            })
        if len(reqs) < size:
            break
    return jobs
...
def _dig(item: dict, path: str):
    ...
    A numeric segment indexes into a list, so 'bulletFields.0' or
    'Locations.0.LocalizedDescription' work (some Workday/UltiPro tenants put the
    location in an array instead of a scalar field).
    """
    if not path:
        return ""
    node = item
