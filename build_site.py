...
<div class="bar">
  <div class="bar-in">
    ...
    <select id="cat">
      ...
      <option value="coredev">★ Software · Data · AI (core dev)</option>
      <option value="software">Software / Full-stack / Dev</option>
      <option value="data">Data (engineer / analyst / scientist)</option>
      <option value="ai">AI / ML</option>
      <option value="cloud">Cloud / DevOps / SRE</option>
      <option value="security">Security</option>
      <option value="qa">QA / Test</option>
      <option value="itops">IT / Support / Infra / ERP</option>
    </select>
    <select id="kind">
...
const q=$('q'), locSel=$('loc'), kindSel=$('kind'), firmSel=$('firm'), fresh=$('fresh'), catSel=$('cat');
...
// Classify a job by title into a role family. Order matters: AI before Data before
// Software (so "AI Engineer" isn't caught as generic dev), specialized IT last.
function category(t){
  const s=(t||'').toLowerCase();
  if(/data scientist|machine learning|\bml\b|ml engineer|ai engineer|\bai\b|artificial intelligence|deep learning|\bnlp\b|\bllm\b|genai|generative ai|computer vision|mlops|applied scientist|ai\/ml|ml\/ai/.test(s)) return 'ai';
  if(/data engineer|data analyst|data analytics|analytics engineer|data architect|database engineer|\bbi\b|business intelligence|\betl\b|data warehouse|big data|data scien|data governance|data management/.test(s)) return 'data';
  if(/software|full[ -]?stack|developer|programmer|front[ -]?end|back[ -]?end|web (developer|engineer)|mobile (developer|engineer)|\bios\b|android|embedded|firmware|\bsde\b|\bswe\b|application (engineer|developer|architect)|api engineer|game develop|software architect/.test(s)) return 'software';
  if(/devops|site reliability|\bsre\b|platform engineer|cloud (engineer|architect)|infrastructure engineer|kubernetes/.test(s)) return 'cloud';
  if(/security|cyber|soc analyst|\biam\b|\bgrc\b|penetration|vulnerability|infosec/.test(s)) return 'security';
  if(/\bqa\b|quality assurance|test engineer|\bsdet\b|test automation|tester|quality engineer|test analyst/.test(s)) return 'qa';
  return 'itops';   // help desk, network, sysadmin, ERP, Epic/health-IT, IT PM/BA, etc.
}
function metro(loc){
...
function render(){
  ...
  const loc=locSel.value, kind=kindSel.value, firm=firmSel.value, fo=fresh.checked, cat=catSel.value;
  const catOK=j=>{ if(!cat) return true; const c=category(j.title);
                   return cat==='coredev' ? (c==='software'||c==='data'||c==='ai') : c===cat; };
  ...
  listEl.innerHTML = rows.map(j=>{
    ...
    const CATLBL={software:'💻 Software',data:'📊 Data',ai:'🤖 AI/ML',cloud:'☁️ Cloud/DevOps',security:'🔒 Security',qa:'🧪 QA',itops:'🛠️ IT/Infra'};
    const catChip = `<span class="chip">${CATLBL[category(j.title)]||'🛠️ IT'}</span>`;
    const al=ageLabel(d);
    return `<div class="card">
      ...
      <div class="body">
        ...
        <div class="metarow">${newChip}${catChip}${locChip}${kindChip}</div>
      </div>
      <div class="right">
...
[q,locSel,kindSel,firmSel,fresh,catSel].forEach(el=>el.addEventListener('input',render));
render();
</script>
</body>
</html>
