#!/usr/bin/env python3
"""
Ohio Tech Jobs — scrape Ohio employer & staffing-firm career pages for tech roles.
...
def run(only=None, headful=False):
    ...
    needs_http = any(
        f["mode"] in ("api", "oracle", "apify_search", "api_html", "sitemap", "html", "rss") or f.get("detail_date")
        for f in firms
    )
    ...
    if needs_http:
        ...
        try:
            ...
            http.headers.update({"User-Agent": "Mozilla/5.0 OhioTechJobs"})
    ...
    try:
        for firm in firms:
            ...
            try:
                ...
                elif firm["mode"] == "oracle":
                    raw = engine.scrape_oracle(firm, http)
            ...
            if firm.get("detail_date") and http:
                ...
                for j in kept:
                    j["posted"] = engine.normalize_posted(j.get("posted", ""))
                # locations/dates are now populated — re-apply the Ohio/remote gate
                # (the listing scrape had blank locations, so this is where iCIMS-style
                # firms actually get filtered to Ohio) + recency. date-enriched firms
                # have RELIABLE post dates, so drop genuinely-old jobs (default 60d).
                before = len(kept)
                kept = [j for j in kept if filters.keep_location(j.get("location", ""), firm)]
                max_age = firm.get("max_age_days", 60)
                kept = [j for j in kept if engine.within_age(j.get("posted", ""), max_age)]
                if len(kept) != before:
                    print(f"  dropped {before - len(kept)} non-Ohio/stale after enrichment", flush=True)
    ...
    # Tracking: show exactly which jobs are NEW this run (grouped by firm).
    new_jobs = summary.get("new_jobs", [])
    if new_jobs:
        print(f"\n🆕 {len(new_jobs)} new job(s) since last run → data/new_jobs.json")
        by_firm: dict[str, list[dict]] = {}
        for j in new_jobs:
            by_firm.setdefault(j.get("firm", "?"), []).append(j)
        for firm in sorted(by_firm):
            print(f"  {firm}:")
            for j in sorted(by_firm[firm], key=lambda x: x.get("title", "")):
